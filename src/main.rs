...
fn get_local_sink<'a, 'b>(handle: &'a Handle,
                      iface: String,
...
fn update_endpoint(handle: Handle,
                   interface: String,
        ...
        -> Box<Future<Item=(), Error=()>>
{
    ...
    let future = if let Ok(future) = res {
        ...
        Box::new(future.or_else(|e| {
            warn!("err={:?}", e);
            err(e)
        }).map_err(|_| ()).and_then(move |conn| {
...
fn main() {
    ...
    let wg_addr: SocketAddr = format!("127.0.0.1:{}", cfg.interface.listen_port).parse().unwrap();

    let mut core = Core::new().unwrap();
    let handle = core.handle();

    let sinks = Arc::new(Mutex::new(HashMap::<SocketAddr, (SplitSink<UdpFramed<RawCodec>>, SocketAddr)>::new()));
    ...
    let (future, public_sink) = duplicate_sink(public_sink);
    handle.spawn(future);

    let (future, public_stream1, public_stream2) = duplicate_stream(public_stream);
    ...
    let future = dht::stun_publish(handle.clone(), public_sink1, public_stream1, interface.to_string(), remote_key);
    let future = future.map_err(|_| ());
    handle.spawn(future);

    handle.spawn(update_endpoint(handle.clone(), interface.to_string(), remote_key.clone(), public_sink.clone(), sinks.clone()));

    let future = wireguard_dispatch(sinks, public_sink, public_stream2, handle.remote(), interface.to_string(), wg_addr);
    core.run(future).unwrap();
}

fn catch_and_report_error<F,R>(func: F) -> FutureResult<R,()>
    where F: FnOnce() -> errors::Result<R>
{
    match func() {
        Ok(r) => ok(r),
        Err(e) => {
